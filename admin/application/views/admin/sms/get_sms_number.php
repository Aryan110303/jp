...
//                                            $details = $this->Classteacher_model->get_data_by_query1("SELECT * from students  where guardian_phone = '$value'") ;
                                                $details = $this->Classteacher_model->get_phone_no($value,$class_id,$section_id) ;
                                                if (empty($details)) {
//                                                $details = $this->Classteacher_model->get_data_by_query("SELECT * from students  where mobileno = '$value'  ") ;//get_father_no
                                                 $details = $this->Classteacher_model->get_father_no($value,$class_id,$section_id) ;
                                                }
                                                
                                               $class_details =  $this->Classteacher_model->get_data_by_query1("SELECT * from student_session  where student_id = '".$details['id']."'  ") ;
                                            ...
                                            ?>
                                             ...
                                             <td><?=  $this->Classteacher_model->findfield('classes','id',$class_details['class_id'],'class'); ?></td>
                                             <td><?=  $this->Classteacher_model->findfield('sections','id',$class_details['section_id'],'section'); ?></td>
                                             <td> <?= $num_details['created_at'] ?></td>
                                              <td class="text-center"> <?php if ($num_details['updated_at']  == '0000-00-00 00:00:00') {
                                                 echo '--' ;
                                              } else 
                                               echo $num_details['updated_at'] ; ?></td>
                                              <td> <?php if ($num_details['sms_status'] == '1') {
