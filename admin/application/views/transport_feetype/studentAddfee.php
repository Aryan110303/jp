...
if (isset($_REQUEST['print_hdn']) && $_REQUEST['print_hdn'] != '') {
...
<div class="content-wrapper">    
    <section class="content-header">
        <h1>
            <i class="fa fa-money"></i> Transport Fee collection<small><?php echo $this->lang->line('student_fee'); ?></small></h1>
    </section>
                    ...
                    <div class="box-header">
                        <div class="row">
                            ...
                            <div class="col-md-8 ">
                                ...
                                <div class="btn-group pull-right">
                                    <a href="<?php echo base_url() ?>transportfee/editfee/" type="button" class="btn btn-primary btn-xs">
                                        <i class="fa fa-arrow-left"></i> <?php echo $this->lang->line('back'); ?></a>
                                </div>
                            </div>
                        </div>  
                                        ...
                                        if ($feetype_balance > 0) {
                                            ...
                                            <td class="text text-left">
                                                <?php if ($feetype_balance != 0): ?>
                                                    <input type="checkbox" name="fess_checked[]" value="<?php echo $value_fee->id ?>" />
                                                <?php endif; ?>
...
//                                 }
                        ...
                        <div class="row no-print">
                            <div class="col-xs-12 table-responsive">
                                <button  class="btn btn-xs btn-info "  data-toggle="modal" data-target="#myFeesModalbulk" onclick="bulkpay();" ><i class="fa fa-money"></i> Bulk Pay  </button>
                            </div>
                        </div>
                    </div>
                    <!-- /.box-body -->
            ...
            <div class="modal-body pb0">
                <div class="form-horizontal">
                    <div class="box-body">
                        ...
                        <input  type="hidden" class="form-control" id="student_session_id" value="<?php echo $value_fee->student_session_id?>" readonly="readonly"/>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-9">
                                <textarea class="form-control" rows="3" id="description" placeholder=""><?php echo $value_fee->due_date; ?></textarea>
                            </div>
                        </div>
                    </div>                   
                </div>
...
<form action="" method="post" name="frm_hdn" id="frm_hdn" enctype="multipart/form-data">
<input type="text" name="print_hdn" id="print_hdn" value=""/>
</form>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        $(document).on('click', '.printDoc', function () {
            ...
            $.ajax({
                url: '<?php echo site_url("transportfee/printFeesByName") ?>',
                type: 'post',
        ...
        $(document).on('click', '.printInv', function () {
            ...
            $.ajax({
                url: '<?php echo site_url("transportfee/printFeesByGroup") ?>',
                type: 'post',
...
//        var fee_groups_feetype_id = $('#fee_groups_feetype_id').val();
        ...
        $.ajax({
            ...
            type: 'post',
            data: {date: date, type: feetype, amount: amount, amount_discount: amount_discount, amount_fine: amount_fine, description: description, student_fees_master_id: student_fees_master_id,student_session_id: student_session_id, payment_mode: payment_mode, guardian_phone: guardian_phone, guardian_email: guardian_email},
            dataType: 'json',
            success: function (response) {
                $this.button('reset');
                if (response.status == "success") {
                    $.ajax({
                        url: '<?php echo site_url("transportfee/printFeesByGroup") ?>',
                        type: 'post',
                        data: {'fee_master_id': student_fees_master_id},
                        success: function (response) {
                            Popup(response);
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.status == "success") {
                    ...
                    $('#amount_fine').val(data.fine_amount);
                    $('#myFeesModal').modal({
                        backdrop: 'static',
                        keyboard: false,
                        show: true
...
<script type="text/javascript" lang="">
    function bulkpay() {
        var array_to_pay = [];
        $("input[name^='fess_checked']:checked").each(function () {
            //console.log($(this).val());
            //var fee_master_id = $(this).value;
            array_to_pay.push($(this).val());

        });
        if (array_to_pay.length == 0) {
            alert("no record selected");
        } else {

            $.ajax({
                url: '<?php echo site_url("transportfee/geBalanceFee3") ?>',
                type: 'post',
                data: {'data': JSON.stringify(array_to_pay)},
                dataType: 'JSON',
                success: function (data) {
                    console.log(data);
                    if (data.status == "success") {

                        $('#amount_lst').val(data.balance);
                        $('#amount_fine_lst').val(data.amount_fine);
                        $('#student_fees_master_id_list').val(data.master_list);
                        $('#myFeesModalbulk').modal({
                            backdrop: 'static',
                            keyboard: false,
                            show: true
                        });
...
<script type="text/javascript">
    $(document).on('click', '.save_button_lst', function (e) {
        var $this = $(this);
        $this.button('loading');
        var form = $(this).attr('frm');
        var feetype = $('#feetype_').val();
        var date = $('#date_lst').val();
        var amount = $('#amount_lst').val();
        //alert(amount);  

        var amount_discount = $('#amount_discount_lst').val();
        var amount_fine = $('#amount_fine_lst').val();
        var description = $('#description_lst').val();
        var guardian_phone = $('#guardian_phone').val();
        var guardian_email = $('#guardian_email').val();
        var student_fees_master_id = $('#student_fees_master_id_list').val();
//      var fee_groups_feetype_id = $('#fee_groups_feetype_id').val();
        var payment_mode = $('input[name="payment_mode_fee_lst"]:checked').val();
        $.ajax({
            url: '<?php echo site_url("transportfee/addstudentfee_lst") ?>',
            type: 'post',
            ...
            success: function (response) {
                ...
                if (response.status == "success") {
                    ...
                    $.ajax({
                        url: '<?php echo site_url("transportfee/printFeesByGroupArray_3") ?>',
                        type: 'post',
                        data: {'data': student_fees_master_id},
                        success: function (response) {
//                        alert(response);
                            $('#print_hdn').val(response);
                            $('#frm_hdn').submit();
                            //$('#print_hdn').html(response);
                        }
                    });
...
<?php if (isset($_REQUEST['print_hdn']) && $_REQUEST['print_hdn'] != '') { ?>
        var data = '<?php echo $_REQUEST['print_hdn']; ?>';
        Popup(data);
    //alert(data);
    // var frame12 = $('<iframe />');
    //        frame12[0].name = "frame1";
    //        frame12.css({"position": "absolute", "top": "-1000000px"});
    //        $("body").append(frame12);
    //        var frameDoc = frame12[0].contentWindow ? frame12[0].contentWindow : frame12[0].contentDocument.document ? frame12[0].contentDocument.document : frame12[0].contentDocument;
    //        frameDoc.document.open();
    //        //Create a new HTML document.
    //        frameDoc.document.write('<html>');
    //        frameDoc.document.write('<head>');
...
<div class="modal fade" id="myFeesModalbulk" role="dialog">
    <div class="modal-dialog">      
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
                <h4 class="modal-title title text-center fees_title">Bulk Fees</h4>
            </div>
            <div class="modal-body pb0">
                <div class="form-horizontal">
                    <div class="box-body">
                        ...
                        <input  type="hidden" class="form-control" id="guardian_phone" value="<?php echo $student['guardian_phone'] ?>" readonly="readonly"/>
                        <input  type="hidden" class="form-control" id="guardian_email" value="<?php echo $student['guardian_email'] ?>" readonly="readonly"/>
                        <input  type="hidden" class="form-control" id="student_fees_master_id_list" value="0" readonly="readonly"/>
                        <div class="form-group">
                            <label for="inputEmail3" class="col-sm-3 control-label"><?php echo $this->lang->line('date'); ?></label>
                            <div class="col-sm-9">
                                <input  id="date_lst" name="admission_date" placeholder="" type="text" class="form-control date"  value="<?php echo date($this->customlib->getSchoolDateFormat()); ?>" readonly="readonly"/>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            <label for="inputPassword3" class="col-sm-3 control-label"><?php echo $this->lang->line('amount'); ?> </label>
                            <div class="col-sm-9">
                                <input type="text" autofocus="" class="form-control modal_amount" id="amount_lst" value="0"  >
                                <span class="text-danger" id="amount_errorLst"></span>
                            </div>
                        </div>
                        ...
                        <div class="form-group mb0">
                            ...
                            <div class="col-sm-9">
                                ...
                                <div class="col-md-5 col-sm-5">
                                    <div class="form-group">
                                        <input type="text" class="form-control" id="amount_discount_lst" value="0">
                                        <span class="text-danger" id="amount_discount_errorLst"></span></div>
                                </div> 
                                <div class="col-md-2 col-sm-2 ltextright">
                                    <label for="inputPassword3" class="row control-label"><?php echo $this->lang->line('fine'); ?></label>
                                </div>
                                <div class="col-md-5 col-sm-5">
                                    <div class="form-group">
                                        <input type="text" class="form-control" id="amount_fine_lst" value="">
                                        <span class="text-danger" id="amount_fine_errorLst"></span>
                                    </div>
                                </div>   
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-9">
                                <label class="radio-inline">
                                    <input type="radio" name="payment_mode_fee_lst" value="Cash" checked="checked"><?php echo $this->lang->line('cash'); ?>
                                </label>
                                <label class="radio-inline">
                                    <input type="radio" name="payment_mode_fee_lst" value="Cheque"><?php echo $this->lang->line('cheque'); ?>
                                </label>
                                <label class="radio-inline">
                                    <input type="radio" name="payment_mode_fee_lst" value="DD"><?php echo $this->lang->line('dd'); ?>
                                </label>
                                <span class="text-danger" id="payment_mode_errorLst"></span>
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="col-sm-9">
                                <textarea class="form-control" rows="3" id="description_lst" placeholder=""><?php echo $value_fee->due_date; ?></textarea>
                            </div>
                        </div>
                    </div>                   
                </div>
            ...
            <div class="modal-footer">
                <div class="box-body">
                    <button type="button" class="btn btn-default pull-left" data-dismiss="modal"><?php echo $this->lang->line('cancel'); ?></button>
                    <button type="button" class="btn cfees save_button_lst" id="load" data-loading-text="<i class='fa fa-circle-o-notch fa-spin'></i> Processing"> <?php echo $currency_symbol; ?> <?php echo $this->lang->line('collect_fees'); ?> </button>
                </div>
            </div>
        </div>
