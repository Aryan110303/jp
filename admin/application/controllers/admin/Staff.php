...
class Staff extends Admin_Controller {
    ...
    function create() {
        ...
        $this->form_validation->set_rules('group', 'Teachers Group', 'trim|required|xss_clean');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $student_session_id=$this->staff_model->add_student_session($data_new);
			
			if($vehicle_id){
                $student_fee=$this->db->get_where('student_session',array('student_id'=>$insert_id,'user_type'=>2))->row();
                $mydate= "10-04-".date('Y');
