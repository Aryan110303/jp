...
class Transport_user extends Admin_Controller {

    function __construct() {
        parent::__construct();
        $this->load->helper('file');
        $this->load->library('mailsmsconf');
        $this->load->model('transport_user_model');
//        $this->lang->load('message', 'english');
       $this->role;
    }

    function delete($id) {
        $data['title'] = 'Transport User List';
        $this->transport_user_model->remove($id);
        redirect('admin/transport_user/index');
    }

    function index() {
        $this->session->set_userdata('top_menu', 'Fees Collection');
        $this->session->set_userdata('sub_menu', 'transport_user/index');
        $data['title'] = 'Add Transport User';
        ...
        if ($this->form_validation->run() == FALSE) {
            $librarian_result = $this->transport_user_model->get();
            $data['librarianlist'] = $librarian_result;
            $genderList = $this->customlib->getGender();
            $data['genderList'] = $genderList;
            $this->load->view('layout/header', $data);
            $this->load->view('admin/transport_user/transportuserList', $data);
            ...
            $insert_id = $this->transport_user_model->add($data);
            $user_password = $this->role->get_random_password($chars_min = 6, $chars_max = 6, $use_upper_case = false, $include_numbers = true, $include_special_chars = false);
            $data_student_login = array(
//                'username' => $this->transport_login_prefix . $insert_id,
                'username' => 'transport_'. $insert_id,
            ...
            if (isset($_FILES["file"]) && !empty($_FILES['file']['name'])) {
                ...
                $this->transport_user_model->add($data_img);
            }
            ...
            $this->session->set_flashdata('msg', '<div class="alert alert-success text-left">Transport user added successfully</div>');
            redirect('admin/transport_user/index');
        }
    }
    ...
    function edit($id) {
        ...
        $data['title'] = 'Edit Transport User';
        $data['id'] = $id;
        $genderList = $this->customlib->getGender();
        $data['genderList'] = $genderList;
        $librarian = $this->transport_user_model->get($id);
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data['librarianlist'] = $librarian_result;
            $this->load->view('layout/header', $data);
            $this->load->view('admin/transport_user/transportuserEdit', $data);
            $this->load->view('layout/footer', $data);
        } else {
            $data = array(
                'id' => $id,
            ...
            $this->session->set_flashdata('msg', '<div class="alert alert-success">Transport user updated successfully</div>');
            redirect('admin/transport_user/index');
        }
    }
    ...
    function view($id) {
        ...
        $accountant = $this->transport_user_model->get($id);

        $data['accountant'] = $accountant;

        $this->load->view('layout/header', $data);
        $this->load->view('admin/transport_user/transportuserShow', $data);
        $this->load->view('layout/footer', $data);
    }
