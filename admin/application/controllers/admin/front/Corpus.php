...
class Corpus extends Admin_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('Cms_corpus_model');
    }

    function index() {
      
        $data = array();
        $listResult = $this->Cms_corpus_model->get_all();

        $data['listResult'] = $listResult;
        $this->load->view('layout/header');
        $this->load->view('admin/front/corpus/index', $data);
     ...
     function create() {
        ...
        if ($this->form_validation->run() == FALSE) {
            $this->load->view('layout/header');
            $this->load->view('admin/front/corpus/create');
            $this->load->view('layout/footer');
        } else {
    ...
    function edit($id) {
        ...
        $result = $this->Cms_corpus_model->getBy_id($id);
        $data['result'] = $result;
        $this->form_validation->set_rules('title', 'title', 'trim|required|xss_clean');
        $this->form_validation->set_rules('description', 'Description', 'trim|required|xss_clean');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->load->view('layout/header');
            $this->load->view('admin/front/corpus/edit', $data);
            $this->load->view('layout/footer');
        } else {
            $data = array(
            ...
            $this->Cms_corpus_model->update($data);
    ...
    function delete($id) {
        $res=$this->Cms_corpus_model->getBy_id($id);
        if($res){

        // $data['title'] = 'Fees Master List';
        $this->Cms_corpus_model->delete($id);
    }
        redirect('admin/front/corpus');
    }
