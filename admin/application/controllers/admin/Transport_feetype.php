...
class Transport_feetype extends Admin_Controller {
...
//        $this->lang->load('message', 'english');
    ...
    function index() {
        ...
        $this->session->set_userdata('sub_menu', 'transport_feetype/index');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            redirect('admin/transport_feetype/index');
        }
        $feegroup_result = $this->transportfeetype_model->get();
        $data['feetypeList'] = $feegroup_result;

        $this->load->view('layout/header', $data);
        $this->load->view('admin/transport_feetype/feetypeList', $data);
    ...
    function edit($id) {
        ...
        if ($this->form_validation->run() == FALSE) {
            $this->load->view('layout/header', $data);
            $this->load->view('admin/transport_feetype/feetypeEdit', $data);
            $this->load->view('layout/footer', $data);
        } else {
            $data = array(
                'id' => $id,
