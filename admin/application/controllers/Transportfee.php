    ...
    public function test($id=null) {
     ...
     public function student_transport_fee() {
        $date=$this->input->post('dob');
         $student_due_fee = $this->transport_custom_model->student_fees_details($date);
         $data['student_due_fee'] = $student_due_fee;
        $this->load->view('layout/header', $data);
        $this->load->view('transport_feetype/studentAcademicReport_1', $data);
     ...
     function addstudentfee() {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $student_array_json = array(
                ...
                'submit_by' => $collected_by
            );
            $result_fee = $this->transport_custom_model->insert_fee($student_array_json);
            $json_array = array(
                'amount' => $this->input->post('amount'),
                'date' => date('Y-m-d', $this->customlib->datetostrtotime($this->input->post('date'))),
                'amount_discount' => $this->input->post('amount_discount'),
