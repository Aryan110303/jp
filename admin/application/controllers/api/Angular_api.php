  ...
  public function Feestaff_login() {
    ...
    {            
     ...
     if ($row != false) {
       ...
       $current_date = $this->getValidDate($todate);
       $session = $this->setting_model->getCurrentSession();
...
public function All_student() {
 ...
 public function houseList()
 {
  ...
  if ($postdata != ''){     
      ...
      if($student_due_fee[0]->name == "Balance Master"){
        if(!empty($student_due_fee[1])){
          $feeA['student_due_fee'] = $student_due_fee[1];
          $feeA['student_previous_due_fee'] = $student_due_fee[0]->fees[0]->amount;
         }else{
          $feeA['student_due_fee'] = $student_due_fee[0];
          $feeA['student_previous_due_fee'] =0;
         }      
      }else{
    
        $feeA['student_due_fee'] = $student_due_fee[0];
        $feeA['student_previous_due_fee'] = ($student_due_fee[1]->fees[0]->amount=='0.00')?0:$student_due_fee[1]->fees[0]->amount;
      }
...
/*    if(!empty($student_due_fee[1])){
  ...
  // print_r($main_arr[0]->name == "Balance Master"); die;

   if($main_arr[0]->name == "Balance Master"){
       $feeA['student_due_fee']= $main_arr[1];
    
  /*   if(!empty($main_arr[1])){
      $feeA['student_due_fee']= $main_arr[1];
     }else{
      $feeA['student_due_fee']=$main_arr[0];
     } */
   }else{    
      $feeA['student_due_fee']= $main_arr[0];     
...
public function cleardues()
{
  ...
  if ($postdata != '' ){ 
   ...
   if(!empty($request)){
    ...
    $restamount = $this->Api_model->checkpreviousBal($student_id,$session_id,$amount);
      if($restamount){
        $outputarr['Status'] = 1; 
        $outputarr['Msg'] = "Amount paid";
      }else{
        $outputarr['Status'] = 1; 
        $outputarr['Msg'] = "Amount not paid";
      }    
    } else { 
    $outputarr['Status'] = 0; 
    $outputarr['Msg'] = "Failed";
  ...
  if (!empty($feeList)) {
    ...
    $msg_data1=array(
         ...
         // 'message'=>"Dear Admin (CAS JABALPUR), Daily Fees Report downloaded please tally the amount, Fee collection amount Rs. $feelist_amount, Fee Reverted amount Rs. $feelist_revert_amount, Total expenses Rs. $feelist_expenses_amount  .",
          'message'=>"Dear Admin (JABALPUR), Total collection amount Rs. $feelist_amount, Fee Reverted amount Rs. $feelist_revert_amount, Total expenses Rs. $feelist_expenses_amount  .",
        ...
        $this->Messages_model->add($msg_data1);
          ...
          $msg_data2=array(
          'title'=>'Daily Fee Report',
          //'message'=>"Dear Admin  (JABALPUR), Daily Fees Report downloaded please tally the amount, Fee collection amount Rs. $feelist_amount, Fee Reverted amount Rs. $feelist_revert_amount, Total expenses Rs. $feelist_expenses_amount  .",
          'message'=>"Dear Admin  (JABALPUR), Total collection amount Rs. $feelist_amount, Fee Reverted amount Rs. $feelist_revert_amount, Total expenses Rs. $feelist_expenses_amount  .",
          'session_id'=>$session_id,
...
echo json_encode($outputarr);
  ...
  public function sendsms_feeDue()
  {
    ...
    if ($postdata != ''){     
      ...
      foreach($request->studentData as $student){
        ...
        if($student_session_id && !empty($mobnumber)){
  ...
  public function MarksGenerator()
  {
    $arr  = array();
    $postdata = file_get_contents("php://input");
    //$postdata =  '{"class_id":"38","section_id":"17","exam_id":"3","session_id":"14"}';    
    if ($postdata != '')
    {  
     $request = json_decode($postdata);
     if(!empty($request))
...
//$postdata ='{"class_id":"38","section_id":"17","session_id":"13"}';
...
//  echo $this->db->last_query();
  ...
  */ if (!empty($exam_subjects)) { 
  ...
  //**************************************************************************** 
    ...
    foreach ($new_array as $first_arr) {
      ...
      elseif ($stuval['class'] == 'VI' || $stuval['class'] == 'VII'|| $stuval['class'] == 'VIII') {
        ...
        $html=$this->load->view('marksheet/marksheet6', $data, true); 
      } 
  ...
  //*****************************************************************************************
        ...
        $html=$this->load->view('marksheet/marksheet5', $data, true); 
      }
//print_r($html); die;
  ...
  public function All_MarksheetlistByClass_old_legal()
  {
    $postdata = file_get_contents("php://input");
     ...
     else
     {
      ...
      if (!empty($mobnumber)) {
        $msg_data2=array(
           ...
           School is coming with the new app 'ACADIANS KONNECT' to Download app please click on the below URL. 
           https://play.google.com/store/apps/details?id=com.central.academy&hl=en 
           Id: Registered Mobile No. 
...
public function get_proxy_teacher()
 {
    $postdata = file_get_contents("php://input");
    //$postdata ='{"proxydate": "2019-12-12", "session_id": "14", "id": "173", "type": "3", "teacher_name": "RUCHIKA KHANNA"}';
     if(!empty($postdata))
     {
       $request = json_decode($postdata);
        if(!empty($request))
 ...
 {          
     ...
     /* echo "<pre>";
      ...
      $minlecture = 1 ;
     $freeteachers = $this->db->query("SELECT staff.id , staff.name , staff.surname ,count(teacher_id) as countid from staff left join teacher_timetable on staff.id = teacher_timetable.teacher_id
       where staff.teachers_group != 0 
       and staff.teachers_group in ($teacher_group) 
       and teacher_timetable.class_id = 0 
       and teacher_timetable.section_id = 0 
       and teacher_timetable.subject_id = 0 
       and days = $day and staff.id not in ( $busyteachers )
       and staff.id not in ($engagged_teacher) 
       and staff.id not in (SELECT extrateacher_id from proxyteachers where `date` = '".$date."' and period_id = $period ) 
       and staff.id not in (SELECT extrateacher_id from proxyteachers where `date` = '".$date."'
        group by extrateacher_id having count(extrateacher_id) >= 6) 
       group by teacher_timetable.teacher_id having count(teacher_timetable.teacher_id) > $minlecture order by staff.teachers_group desc ")->result();

/* $freeteachers = $this->db->query("SELECT staff.id , staff.name , staff.surname ,count(teacher_id) as countid from staff left join teacher_timetable on staff.id = teacher_timetable.teacher_id where staff.teachers_group != 0 and staff.teachers_group in ($teacher_group) and teacher_timetable.class_id != 0 and teacher_timetable.section_id != 0 and teacher_timetable.subject_id != 0 and days = $day and staff.id not in ( $busyteachers ) and staff.id not in ($engagged_teacher) and staff.id not in (SELECT extrateacher_id from proxyteachers where `date` = '".$date."' and period_id = $period group by extrateacher_id HAVING COUNT(extrateacher_id) < 6) group by teacher_timetable.teacher_id having count(teacher_timetable.teacher_id) <= $minlecture ")->result();*/
   ...
   $newval = $grp ;
        if ($grp > $min) {
          for ($i=1; $i <= $max; $i++){ 
             ...
             $result2 = array_merge($result1, $post);
             $result = array_merge(array(7),$result2);
           }else{
              $result = $group ;
             }  
   return $result ;
      ...
      {     
        $request = json_decode($postdata);
               if (!empty($request)) { 
                  ...
                  if ($this->db->insert_id()) {
                    $outputarr['Status'] = 1; 
                    $outputarr['Msg'] = "Data inserted Successfully";
                  }else{
                    $outputarr['Status'] = 0; 
                    $outputarr['Msg'] = "Insertion failed ";
                  }
                }
                else{
                  $outputarr['Status'] = 0; 
...
public function deleteEnquiry()
{
   ...
   $enquiryList = $this->db->query("SELECT * from visitors_book where 1")->result() ; 
  if (!empty($enquiryList)) {
            $outputarr['Result'] = $enquiryList;
            $outputarr['Status'] = 1;
            $outputarr['Msg'] = " Data found ";
     }else{
       $outputarr['Status'] = 0;
