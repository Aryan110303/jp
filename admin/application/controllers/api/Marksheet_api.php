  ...
  class Marksheet_api extends CI_Controller {
  ...
  public function gatepass_list($value='')
  {
    ...
    $result = $this->Api_model->get_Gatepass_list($page,$session_id);
    if (!empty($result)) {
      $outputarr['Status'] = 1; 
  ...
  public function get_student_gp()
  {
    ...
    if ($postdata != '') {  
      ...
      $session_id = $request->session_id ;
      if ( $rollno != '') { 
        $result = $this->Api_model->get_student_gp($rollno,$session_id);
      } 
      if (!empty($result)) {
        $outputarr['Status'] = 1; 
        $outputarr['Msg'] = "Success";
  ...
  public function add_gatepass()
  {
   ...
   if ($postdata != '') {     
    ...
    $add['session_id'] = $request->session_id ;
 ...
 public function houseList()
 {
  ...
  if ($postdata != ''){     
     ...
     $feeA['feesdate'] = $this->getValidDate($date1);
     $feeA['LateFee'] = 50;
    ...
    {  
     ...
     {  
      ...
      //$session_id= 13;
       ...
       $studentList  = $this->student_model->searchByClassSection_api($class_id, $section_id, $session_id);  
       ...
       if(!empty($examSchedule)){
        ...
        foreach ($studentList as $stu_key => $stu_value) {
          ...
          $array['totaldays'] = $stu_value['totaldays'];
          $array['presentdays'] = $stu_value['presentdays'];
            $array['student_session_id'] = $stu_value['student_session_id'];
          $grade= $this->student_model->get_grade_student($stu_value['admission_no'],$session_id); 
          if (empty($grade)) {
    ...
    {  
     ...
     {            
      ...
      foreach ($student_array as $key => $student) {
          if ($student->student_session_id > 0 ) {
              $totaldays = $student->totaldays;
              $presentdays = $student->presentdays;
              $student_session_id = $student->student_session_id;
           $this->db->where('id',$student_session_id);
           $this->db->update('student_session',array('totaldays'=>$totaldays,'presentdays'=>$presentdays));
          }
...
public function MarksheetlistByClass()
  ...
  public function Marksheetnew()
  {
    $postdata = file_get_contents("php://input");
   //$postdata ='{"class_id":"38","section_id":"17","session_id":"14"}';
    if ($postdata != '')
    { 
      $request = json_decode($postdata);
      if(!empty($request))
      { 
        ...
        $session_id=$request->session_id ;
        $foldername = $class.$section.$session_id ;
        $folder = FCPATH . "uploads/marksheetPdf/".$foldername;
        if (!is_dir($folder)) {
  ...
  $studentlist = $this->Student_model->searchByClassSection_api($class, $section,$session); 
...
if (!empty($examList)) {
  foreach ($studentlist as $key => $stuval){
    ...
    $marks=[];
    $new_array = array();
    //$code = $studetails['scholar_no'].$session_id;
    $uniquecode = $session_id.$stuval['class_id'].$stuval['section_id'].$stuval['scholar_no'] ;
    $code = md5(uniqid($uniquecode, true));
    /*QR-Image Generation*/
    $PNG_TEMP_DIR = FCPATH.DIRECTORY_SEPARATOR.'/uploads/marksheetqrcode/'.DIRECTORY_SEPARATOR;
    $PNG_WEB_DIR = 'uploads/marksheetqrcode/';
    require_once( APPPATH.'/third_party/qrcode/qrcode/qrlib.php' );
    $errorCorrectionLevel = 'S'; 
    $matrixPointSize = 6;
    $datum=base_url('api/Marksheet_api/getpdf/').$code;
    ...
    foreach ($examList as $ex_key => $ex_value) {
      ...
      $studetails['class_id'] = $stuval['class_id'];
      $studetails['section_id'] = $stuval['section_id'];
      ...
      $studetails['total_days'] = $stuval['totaldays'];
      $studetails['present_days'] = $stuval['presentdays'];
      $studetails['qrimage'] = $qrimage ;
      $exam_subjects = $this->examschedule_model->getresultByStudentandExam_pdf($exam_id, $stuval['id'],$session_id);
      $exam_subjects_1[] = $exam_subjects ;
     if (!empty($exam_subjects)) { 
        $full_marks=0;
        $get_marks=0;
        foreach ($exam_subjects as $key => $value) {
          $childData = $this->examschedule_model->getresultByStudentandExam_pdf_child($exam_id, $stuval['id'],$value['sub_parent_id'],$session_id);
          $full_marks =0 ;
          $get_marks = 0 ;
          if (!empty($childData)) {       
            foreach ($childData as $key => $childs){
              $full_marks += $childs['full_marks'];
              $get_marks +=  $childs['get_marks'];
            }
          }
          $exam_array = array();
          $exam_array['exam_schedule_id'] = $value['exam_schedule_id'];
          ...
          if ($value['optional_status'] == 1 ){
            $exam_array['full_marks'] = 0 ;
               ...
               $exam_array['full_marks'] = $value['full_marks'] + $full_marks;
              }
          $exam_array['passing_marks'] = $value['passing_marks'];
          $exam_array['exam_subject_name'] = $value['name'];
          $exam_array['exam_type'] = $value['type'];
          $exam_array['attendence'] = $value['attendence'];
          $exam_array['get_marks'] = $value['get_marks'] + $get_marks;
          $subject[] = $exam_array['exam_subject_name'];
          if ($value['type'] == 'Written' || $value['type'] == 'Theory') {
            $subject11[] = $exam_array['exam_subject_name'];
            $ele_subject[] = $exam_array['exam_subject_name'];
          }         
          $x[] = $exam_array;
        }
    ...
    $grading = $this->student_model->get_grade_student($studetails['scholar_no'],$session_id); 
    $sub = array();
    ...
    if($stuval['class'] == 'IX'){
     ...
     foreach ($new_array as $first_arr) {
        if($first_arr['exam_name']== 'Annual Examination')
        {
      ...
      foreach ($sub_array_marks as $key1 => $valuesort) {
        ...
        if ($cary == 3){
           $highmark[$key1]= round(($valuesort[2]+$valuesort[1])/2,2);

          /*$vl = round((20 *$valuesort[2])/80 ,2);
          $highmark[$key1]= round(($valuesort[1] + $vl)/2 ,2);*/
        }
        if ($cary == 2) {
          /*$vl = round((20 *$valuesort[1])/80 ,2);
          $highmark[$key1]= round(($valuesort[0] + $vl)/2, 2);*/
           $highmark[$key1]= round(($valuesort[0]+$valuesort[1])/2,2);
        } 
      } 
     // $grading = $this->student_model->get_grade_student($studetails['scholar_no']); 
      $grading = $this->student_model->get_grade_student($studetails['scholar_no'],$session_id); 
      ...
      $html=$this->load->view('marksheet/marksheet9', $data, true); 
    } 
      ...
      $sub = array();
        ...
        // print_r($new_array);
      foreach ($new_array as $first_arr) { 
        $eleventhsubject = array();
        foreach ($first_arr['exam_result'] as $first_arr1){
          if ($first_arr1['exam_type'] == 'Written' || $first_arr1['exam_type'] == 'Theory') {
                ...
                if ($first_arr1['full_marks'] > 0 ) {
                   $eleventhsubject[] = $first_arr1['exam_subject_name'] ; 
                }               
             }else{
               $subprac[$first_arr1['exam_subject_name']]['full'] = intval($first_arr1['full_marks']) ;
               $subprac[$first_arr1['exam_subject_name']]['get'] =  intval($first_arr1['get_marks']);                
              }
        }
        $exam_arr[$first_arr['exam_name']] = $sub;
        $exam_arrprac[$first_arr['exam_name']] = $subprac;
      }
      ...
      $data = array(
        ...
        'quaterlyprac'=> $exam_arrprac['Quarterly Examination'],        
        'halfyearly'=> $exam_arr['Half Yearly Examination'],
        'halfyearlyprac'=> $exam_arrprac['Half Yearly Examination'],
        'final'=> $exam_arr['Annual Examination'],
        'finalprac'=> $exam_arrprac['Annual Examination'],
        'sessionname'=>$session ,
        'subject'=>  $eleventhsubject,
        'grade'=> $grading, 
      );

      
      $html=$this->load->view('marksheet/marksheet11', $data, true); 
    } 
    ...
    elseif ($stuval['class'] == 'VI' || $stuval['class'] == 'VII'|| $stuval['class'] == 'VIII') {
      ...
      $periodicI=$exam_arr1['Periodic Assessment - 1'];
      $periodicIII=$exam_arr1['Periodic Assessment - 2'];
      $periodicII=$exam_arr1['Half Yearly Examination'];
      $final_term=$exam_arr1['Annual Examination'];
      ...
      $data = array(
        ...
        'sessionname'=> $studetails['session'],
        'grade'=> $grading,      );
      $html=$this->load->view('marksheet/marksheetVI', $data, true); 
      ...
      $html=$this->load->view('marksheet/marksheet5new', $data, true); 
    }

    $m_pdf = new \Mpdf\Mpdf(['format' => 'Legal','margin_left' => 0,'margin_right' => 0,'margin_top' => 0,'margin_bottom' => 0,'margin_header' => 0,'margin_footer' => 0]);
    $m_pdf->SetDisplayMode('real');
    $m_pdf->WriteHTML($html);
    $m_pdf->Output($folder.'/'.$code.".pdf", 'F');
    ///die('PRANAV');
    $qrcode['stud_status'] = 0;
    $qrcode['stud_code'] = $code;
    $qrcode['stud_qrimage'] = $qrimage;
    $qrcode['stud_session_id'] = $session_id; 
    $qrcode['stud_section_id'] = $studetails['section_id'];
    $qrcode['stud_class_id'] = $studetails['class_id'];
    $qrcode['stud_admission_no'] = $studetails['scholar_no'];

    $ary['stud_session_id'] = $session_id; 
    $ary['stud_section_id'] = $studetails['section_id'];
    $ary['stud_class_id'] = $studetails['class_id'];
    $ary['stud_admission_no'] = $studetails['scholar_no'];

    $updateary['stud_code'] = $code;
    $updateary['stud_qrimage'] = $qrimage;

    $res = $this->db->select('*')->from('marksheet_qr_data')->where($ary)->get()->row();
        if (empty($res)) {
          $this->db->insert('marksheet_qr_data',$qrcode);
        }else{
          $this->db->where('id',$res->id);
          $this->db->update('marksheet_qr_data',$updateary);
        } 

  }
  $datass['Status'] = 1;
  $datass['Msg']="All Marksheet Generated Successfully"; 
}else{
  $datass['Msg'] = 'Exam Not Found';
  $datass['Status'] = 0;
}
}else{
  $datass['Msg'] = 'Post Data Not Found';
  $datass['Status'] = 0;
}
...
public function getpdf($code)
{
  ...
  $this->load->helper('download');
   $resData = $this->db->query("SELECT * from marksheet_qr_data where stud_code = '$code'")->row(); 
  
  if (!empty($resData)) {
     $path = FCPATH.'uploads/marksheetPdf/'.$resData->stud_class_id.$resData->stud_section_id.$resData->stud_session_id.'/'.$resData->stud_code.'.pdf' ;
...
//================================================================//
  ...
  public function MarksheetByStudent()
  {
    $postdata = file_get_contents("php://input");
    if ($postdata != '')
    { 
 ...
 $studentlist = $this->Student_model->getdetailsByStudentSessionid($student_id); 
  ...
  //*****************************************************************************************
    ...
    $datass['Status'] = 1;
    $datass['zippath']=base_url().'uploads/marksheetPdf/$code.pdf'; 
  }else{
    $datass['Msg'] = 'Exam Not Found';
    $datass['Status'] = 0;
  }
               ...
               $this->studentfee_model->remove($invoice_id, $sub_invoice);
                ...
                $array['Status'] = 1 ;  
                  if (!empty($number)) 
                  {
                        $msg_data=array(
                        'title'=>'Fees Reverted',
                        'message'=>"Dear Parent, your ward Fees has been Reverted (Rollback), for any queries contact School Management - Central Academy Jabalpur.",
                        'session_id'=> $session_id ,
                        'user_list'=>$number,
                        'student_session_id'=>0,
                        );
 ...
 if (!empty($mobileno)) {
     ...
     if ($student['rte'] != 'Yes'){           
            if(!empty($feesession_group_id->id)){
              $feesession_group_ida= $feesession_group_id->id;                                       
                ...
                $data['msg']  = 'Student Added Successfully ! Fees Not Assigned For RTE Student';              
                $data['Status']  = 1; 
                $data['fees_result']  = array(); 
                $data['student_session_id']  = $student_session_id_new;  
                $data['promotedate']  =  $promotedate; 
    }
  ...
  public function PromoteNow_7eye()
  {
   $postdata = file_get_contents("php://input");
...
public function get_proxy_teacher()
 {
    $postdata = file_get_contents("php://input");
   //  $postdata ='{"proxydate": "2019-10-03", "session_id": "14", "id": "173", "type": "3", "teacher_name": "RUCHIKA KHANNA"}';
     if(!empty($postdata))
     {
       $request = json_decode($postdata);
        if(!empty($request))
...
//////////////------------------------------------------------------//////////////
public function get_proxy_teacher_14oct()
 {
    $postdata = file_get_contents("php://input");
 ...
 public function get_proxy_14_oct($class_id=0, $subject=0, $period=0, $day=0 ,$date=0,$section=0,$teacher_id, $teacher_group,$engagged_teacher)
 {        
         ...
         if (!empty($request)) { 
          ...
          $students = $request->user_list;
            foreach ($students as $key => $student) {
               ...
               $data['user_list'] = $student->guardian_phone;
               $data['sms_status'] = $sms_status;
               $data['type'] = 1;
               $data['msg_type'] = $msg_type;
               $data['student_session_id'] = $student->student_session_id;

               $this->db->insert('messages',$data);

                 $outputarr['ddd'] = $this->db->last_query(); 
...
public function AllteacherList()
{
         ...
         if (!empty($request)) { 
          ...
          $teachers = $request->user_list;
            foreach ($teachers as $key => $teacher) {
               ...
               $data['section_id'] = 0;
               $data['session_id'] = $session_id;
               $data['user_list'] = $teacher->contact_no;
               $data['sms_status'] = $sms_status;
               $data['type'] = 1;
               $data['msg_type'] = $msg_type;
               $data['student_session_id'] = 0;
   ...
   public function remainMsgBal()
   {

        $ch = curl_init('http://login.heightsconsultancy.com/API/WebSMS/Http/v1.0a/index.php?method=request_routeid&username=casjbp&password=password&route_id=113&format={json|text}');
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
           $response = curl_exec($ch); 
           curl_close($ch); 
        
           $array_a = @unserialize(trim($response));   
        
       //print_r($array_a); die;
      
            $outputarr['countavailable'] =$array_a[113]['countavailable'] ; 
            $outputarr['countcredit'] =$array_a[113]['countcredit'] ; 
            $outputarr['countconsumed'] =$array_a[113]['countconsumed'] ; 
            $outputarr['Status'] = 1; 
            $outputarr['Msg'] = "Data Found"; 
          echo json_encode($outputarr);
   }

  public function StudentByClassSection()
  {
      $postdata = file_get_contents("php://input");
     if ($postdata != '')
      {     
        $request = json_decode($postdata);
         ...
         $session_id = $request->session_id; 
          if ($request->section_id) {
            $section_id =$request->section_id ;
            $outputarr['result'] =  $this->Api_model->get_student_ByclassSection($class_id, $section_id, $session_id);
          }else{
              $outputarr['result'] =  $this->Api_model->get_student_Byclass($class_id, $session_id);
          }     
    ...
    $postdata = file_get_contents("php://input");
     if ($postdata != '')
      ...
      $session_id = $request->session_id ; 
        ...
        $students = $this->db->query("SELECT students.*,student_session.id as student_session_id, student_session.session_id,classes.class,sections.section from students join student_session on student_session.student_id = students.id join classes on classes.id = student_session.class_id join sections on sections.id = student_session.section_id where students.is_active = 'yes' and student_session.session_id = $session_id ")->result();
      
        $staffs = $this->db->query("SELECT * from staff where is_active = 1")->result();
        $count = 0 ;
        if (!empty($students)) {
         foreach ($students as $key => $student) {
            $count++ ;
                     ...
                     $data['section_id'] =$student->section_id;
                     $data['session_id'] = $session_id;
                     $data['user_list'] = $student->mobileno;
                     $data['sms_status'] = 0;
                     $data['type'] = 1;
                     $data['msg_type'] = $msg_type;
                     $data['student_session_id'] = $student->student_session_id;
      ...
      if (!empty($staffs)) {
         ...
         foreach ($staffs as $key => $staff) {
             $count++ ;
                     ...
                     $data['section_id'] = 0;
                     $data['session_id'] = $session_id;
                     $data['user_list'] = $staff->contact_no;
                     $data['sms_status'] = 0;
                     $data['type'] = 1;
                     $data['msg_type'] = $msg_type;
                     $data['student_session_id'] = 0;
               ...
               $outputarr['Status'] = 1; 
               $outputarr['Msg'] = "Total  ".$count." Messages Sent Successfully ";
...
public function sendMessageother()
{
    $postdata = file_get_contents("php://input");
     if ($postdata != '')
      {     
        $request = json_decode($postdata);
               if (!empty($request)) { 
                  ...
                  foreach ($userlist as $key => $value) {
                     ...
                     $data['section_id'] = 0;
                     $data['session_id'] = $session_id;
                     $data['user_list'] = $value->contact_number;
                     $data['sms_status'] = 0;
                     $data['type'] = 1;
                     $data['msg_type'] = $msg_type;
                     $data['student_session_id'] = 0;
                    ...
                    $outputarr['Status'] = 1; 
                    $outputarr['Msg'] = " Messages Sent Successfully ";
               }else{
                   $outputarr['Status'] = 0; 
                   $outputarr['Msg'] = "Request Data not found ";
               }
...
{   
  if (empty($date)) {
    ...
    if ($current_date < $date) {
      ...
      $checkisholiday = $this->db->query("SELECT * from school_holidays where school_holidays.date = '$current_date'")->row();
    

     if (!empty($checkisholiday)) {
          $this->getValidDate(date('Y-m-d', strtotime('+1 day', strtotime($current_date))));
...
public function getmarksheetlink()
{
  $postdata = file_get_contents("php://input");
  ...
  {  
    ...
    if(!empty($request))
    { 
      $scholar_no = $request->admission_no ;
      $session = $request->session_id ;
      $link =$this->db->query("SELECT stud_code from marksheet_qr_data where stud_admission_no = '".$scholar_no."'")->row()->stud_code; 
    
      $studetails = $this->db->query("SELECT student_session.class_id ,student_session.section_id from student_session 
      join students on students.id = student_session.student_id where students.admission_no = '".$scholar_no."' and student_session.session_id = $session ")->row();
      //print_r($link); die;
      $cls = $studetails->class_id ;  
      $sec = $studetails->section_id ;  
      $var=  $scholar_no.$session ;
      $file = FCPATH . "uploads/marksheetPdf/".$cls.$sec.$session.'/'.$link.'.pdf';
      if(file_exists($file)){
        $data['link'] = base_url("uploads/marksheetPdf/".$cls.$sec.$session.'/'.$link.".pdf") ;
        $data['msg'] = 'Marksheet Link found' ; 
        $data['Status'] = 1; 
      }else{
        $data['msg'] = 'Link not found' ; 
        $data['Status'] = 0 ; 
      }
