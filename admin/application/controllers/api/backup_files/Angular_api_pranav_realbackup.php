...
//daily report message ends

      /*save message for admin*/
/* 
      $msg_data1=array(
            'title'=>'Daily Fee Report',
            'message'=>"Dear Admin, Daily Fees Report downloaded please tally the amount, Fee collection amount Rs. $feelist_amount, Fee Reverted amount Rs. $feelist_revert_amount, Total expenses Rs. $feelist_expenses_amount  .",
          ...
          $this->load->library('email');
            $msg="Dear Admin, Daily Fees Report downloaded please tally the amount, Fee collection amount Rs. $feelist_amount, Fee Reverted amount Rs. $feelist_revert_amount, Total expenses Rs. $feelist_expenses_amount  .";
      ...
      public function add_admission_fee()
{
 ...
 { 
   $teacherList =  $this->Api_model->allteachers();
   $outputarry['msg']  = 'Empty Posted Data';              
   $outputarry['Status']  = 1; 
   $outputarry['teacherList']  = $teacherList; 
    echo json_encode($outputarry);
...
public function duefee_by_Month_olddddd()
  {
    $postdata = file_get_contents("php://input");      
          //$postdata='{"month":"08","session_id":"13"}';
        //  $postdata = $this->input->post('data');
       if ($postdata != ''){   
            $request = json_decode($postdata); 
        if (!empty($request)){        
