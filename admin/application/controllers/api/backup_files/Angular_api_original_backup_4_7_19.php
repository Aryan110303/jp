...
public function get_proxy_teacher()
 {
   ...
   $postdata = file_get_contents("php://input");
  //  $postdata ='{"proxydate":"2019-06-27","id":"154"}' ;
   if(!empty($postdata))
     {
...
if (!empty($freeteachers) && (!in_array($freeteachers->id, $resarry)))
     {
        ...
        if (empty($freeteachers)) {
           $freeteachers = $this->db->query("SELECT staff.id , staff.name , staff.surname ,count(teacher_id) as countid from staff join teacher_timetable on staff.id = teacher_timetable.teacher_id where teacher_timetable.class_id != 0 and teacher_timetable.section_id != 0 and teacher_timetable.subject_id != 0 and days = $day and staff.id not in ($busyteachers) and staff.id not in ($engagged_teacher) and staff.id not in (SELECT extrateacher_id from proxyteachers where `date` = '".$date."' and period_id = $period=0 )  group by teacher_timetable.teacher_id having count(teacher_id) < $minlecture order by countid asc limit 1 ")->row(); 
              if (!empty($freeteachers)) {
                     $object = new stdClass();
                     $object->name = $freeteachers->name;
                     $object->teacher_id = $freeteachers->id;
    ...
    if(!empty($postdata))
      {
         ...
         if(!empty($request))
            {
// file_put_contents('pranavSahni.html', $postdata);
...
public function Paid_fee_details()
 ...
 public function birthdaywishes()
 {
  //echo "<pre>";
   ...
   $stafflist = $this->db->query("SELECT name, surname, contact_no, dob from staff where contact_no !='' and  (staff.dob ='%".$date1."%' or staff.dob = '%".$date2."%' or staff.dob = '%".$date3."%' or staff.dob = '%".$date4."%' ) ")->result();
