      ...
      {  
       ...
       if(!empty($request))
        {      
            $class_id=$request->class_id ;
            $section_id=$request->section_id ;
            $student_due_fee = $this->studentfee_model->getDueFeeBystudentSection($class_id, $section_id, 13);
...
// print_r($data['all_exam']);  
// print_r($data['final_exam']);  
//**************************************************************************** 
...
$arr3 = $exam_arr['Final Examination'];
array_walk($arr3, function (&$val, $key, $foo) {
$val += $foo[$key];
}, $exam_arr['Periodic-3']);

$arr4 = $exam_arr1['Final Examination'];
array_walk($arr4, function (&$val, $key, $foo) {
$val += $foo[$key];
}, $exam_arr1['Periodic-3']);

$arr5 = $exam_arr['Periodic-I'];
array_walk($arr5, function (&$val, $key, $foo) {
$val += $foo[$key];
}, $exam_arr['Periodic-2']);

$arr6 = $exam_arr1['Periodic-I'];
array_walk($arr6, function (&$val, $key, $foo) {
$val += $foo[$key];
}, $exam_arr1['Periodic-2']);

$final_term=array_merge_recursive($arr3,$arr4);
$periodic=array_merge_recursive($arr5,$arr6);
