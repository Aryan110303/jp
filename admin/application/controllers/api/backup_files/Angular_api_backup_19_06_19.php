  ...
  // $postdata =  $this->input->post('data');    
   //  $postdata= '{"id": "2185", "session_id": "14"}';
    if ($postdata != ''){     
     ...
     /*
     $qry = "Select fine_amount from sch_settings limit 1";
     $amount = $this->db->query($qry)->row();    
     $feeA['LateFee'] = $amount->fine_amount);
      */
     $feeA['LateFee'] = 50;
           ...
           {  
            ...
            $count = 0;
              foreach ($user_type_ary as $key => $value) {
                         ...
                         if($status == '2' && $value->staff_attendance_id){
                             $this->db->where('staff_id',$value->staff_attendance_id);
             ...
             $data['absent_teacher'] = $this->db->query("SELECT staff.* ,staff.id as id,staff_attendance.id as staff_attendanceid, staff_attendance.* from staff_attendance join staff on staff_attendance.staff_id = staff.id where staff_attendance.staff_attendance_type_id = 3 and staff_attendance.date ='".$todaydate."' ")->result(); 
...
public function get_proxy_teacher()
 {
   $postdata = file_get_contents("php://input");
   $postdata ='{"proxydate":"2019-06-04","id":"136"}' ;
   if(!empty($postdata))
     {
       $request = json_decode($postdata);
        if(!empty($request))
            { 
               ...
               $value->proxyteacher = $this->get_proxy($value->class_id,$value->subject_id,$value->period_id,$value->days,$proxydate,$value->section_id);
               }
               ...
               $data['Msg'] = 'Result Found';
               $data['Status'] = 1;
              //$proxyteacher=$this->Api_model->getproxy($request->id);
             } else{
              $data['Msg'] = 'No Request data Found';
              $data['Status'] = 0;
                }
 ...
 public function get_proxy($class=0, $subject=0, $period=0, $day=0 ,$date=0,$section=0)
 {
        $result= array();
        $query1="SELECT teacher_timetable.*, periods.value as period, days.value as day,staff.name, subjects.name as subject, classes.class , sections.section ,staff.surname,staff.id from teacher_timetable join classes on $class = classes.id join subjects on $subject = subjects.id join sections on $section = sections.id join staff on teacher_timetable.teacher_id = staff.id join class_teacher on class_teacher.class_id = $class join periods on periods.id = teacher_timetable.period_id join days on days.id = teacher_timetable.days where class_teacher.class_id = $class and teacher_timetable.period_id = $period and teacher_timetable.days = $day and teacher_timetable.class_id= 0 and teacher_timetable.section_id = 0  and staff.id not in (SELECT extrateacher_id from proxyteachers where period_id = $period and `date` = '".$date."' )  group by teacher_timetable.teacher_id LIMIT 1";        

        $result = $this->db->query($query1)->row();

        if(empty($result)){ 
        $query2="SELECT teacher_timetable.*, periods.value as period, days.value as day, subjects.name as subject, classes.class , sections.section , staff.name, staff.surname, staff.id from teacher_timetable  join classes on $class = classes.id join subjects on $subject = subjects.id join sections on $section = sections.id  join staff on teacher_timetable.teacher_id = staff.id join periods on periods.id = teacher_timetable.period_id join days on days.id = teacher_timetable.days  where teacher_timetable.subject_id= $subject and teacher_timetable.period_id = $period and teacher_timetable.days = $day and teacher_timetable.class_id= 0 and teacher_timetable.section_id = 0  and staff.id not in (SELECT extrateacher_id from proxyteachers where period_id = $period and `date` = '".$date."' )  group by teacher_timetable.teacher_id LIMIT 1";
          $result_1 = $this->db->query($query2)->row();
         $result= $result_1 ;
        }

        if(empty($result_1)){              
          $class_data=$this->db->get_where('classes',array('id' => $class))->row_array();
          $a=$class_data['sequence'];
          
        for($i= $a+1   ;$i<= 15;$i++){ 

          $class_data=$this->db->get_where('classes',array('sequence' => $i))->row_array();
       
          $class=$class_data['id']; 
         
         $query3 ="SELECT teacher_timetable.*, staff.name,subjects.name as subject, classes.class , sections.section , staff.surname, periods.value as period ,days.value as day,staff.id from teacher_timetable join staff on teacher_timetable.teacher_id = staff.id join class_teacher on class_teacher.class_id = $class  join classes on $class = classes.id join subjects on $subject = subjects.id join sections on $section = sections.id  join periods on periods.id = teacher_timetable.period_id join days on days.id = teacher_timetable.days where class_teacher.class_id = $class and teacher_timetable.period_id = $period and teacher_timetable.days = $day and teacher_timetable.class_id= 0 and teacher_timetable.section_id = 0  and staff.id not in (SELECT extrateacher_id from proxyteachers where period_id = $period and `date` = '".$date."' )  group by teacher_timetable.teacher_id LIMIT 1"; 

           $result_2 = $this->db->query($query3)->row();
         ...
         if(!empty($request))
            {
              ...
              foreach ($arrry as $key => $val) {
                ...
                $ary = array('days'=> ($val->day)?$val->day:0,'periods'=> $val->period,'classes'=> $val->class,'sections'=> $val->section,'subjects'=> ($val->subject)?$val->subject:0,'extrateacher'=> $val->extrateacher,'on_behalf'=> $val->on_behalf,'date'=> $val->date,'extrateacher_id'=> $val->extrateacher_id,'period_id'=> $val->period_id) ;
   ...
   if(!empty($postdata))
      {
        $request = json_decode($postdata);
         if(!empty($request))
            {
            $ip = $request->ip ; 
              $ip_avail =  $this->db->query("SELECT ip from ipaddress where FIND_IN_SET('".$ip."',ip) > 0 ")->row();
         ...
         if(!empty($request))
            {
               ...
               $data['scholar_no'] = $request->scholar_no ; 
               $this->db->insert('student_answersheet',$data);
               $data['Msg'] = "Data Inserted Successfully";
               $data['status'] = 1;
             }
            else{
