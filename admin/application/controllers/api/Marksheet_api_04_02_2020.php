...
public function MarksheetlistByClass()
  ...
  {
    $postdata = file_get_contents("php://input");
    $postdata ='{"class_id":"33","section_id":"18","session_id":"14"}';
    if ($postdata != '')
    { 
      $request = json_decode($postdata);
      if(!empty($request))
...
//$code = $studetails['scholar_no'].$session_id;
    ...
    foreach ($examList as $ex_key => $ex_value) {
      ...
      if (!empty($exam_subjects)) { 
        ...
        $get_marks=0;
        foreach ($exam_subjects as $key => $value) {
          $childData = $this->examschedule_model->getresultByStudentandExam_pdf_child($exam_id, $stuval['id'],$value['sub_parent_id']);
          $full_marks =0 ;
          $get_marks = 0 ;
          if (!empty($childData)) {       
    ...
    if($stuval['class'] == 'IX'){
     ...
     foreach ($new_array as $first_arr) {
      ...
      $sub = array();
      $all_subjects = array_unique($subject11);
      foreach ($new_array as $first_arr) { 
        foreach ($first_arr['exam_result'] as $first_arr1){
      ...
      $data = array(
        ...
        'finalprac'=> $exam_arrprac['Final Examination'],
    ...
    elseif ($stuval['class'] == 'VI' || $stuval['class'] == 'VII'|| $stuval['class'] == 'VIII') {
     ...
     // print_r($new_array); die; 
      ...
      $periodicII=$exam_arr1['Periodic Assessment - 2'];
      $periodicIII=$exam_arr1['Half Yearly Examination'];
...
public function getmarksheetlink()
{
  ...
  {  
    ...
    { 
      ...
      $studetails = $this->db->query("SELECT student_session.class_id ,student_session.section_id from student_session join students on students.id = student_session.student_id where students.admission_no = $scholar_no and student_session.session_id = $session ")->row();
