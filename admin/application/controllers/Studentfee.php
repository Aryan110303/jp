...
class Studentfee extends Admin_Controller {
    ...
    public function onlineTransactionReport()
    {    
        if($_POST){
            $fromdate = $this->input->post('datefrom');
            $todate = $this->input->post('dateto');
            if(!empty($todate))
                $condition = "transaction_details.created_at >= '$fromdate' and  transaction_details.created_at <= '$todate'";
            else
               $condition = "transaction_details.created_at >= '$fromdate'";        
       
        $data['transactions'] = $this->db->query("SELECT transaction_details.*,students.firstname,students.admission_no,students.lastname,classes.class,sections.section 
        from transaction_details 
        join students on students.id = transaction_details.student_id
        join classes on classes.id = transaction_details.class_id
        join sections on sections.id = transaction_details.section_id
    ...
    $this->load->view('layout/header', $data);
    $this->load->view('studentfee/onlinepayment', $data);
    $this->load->view('layout/footer', $data);
    }


    
    public function attemptOnlineTransaction()
    {    
        if($_POST){
            $fromdate = $this->input->post('datefrom');
            $todate = $this->input->post('dateto');
            if(!empty($todate))
                $condition = "temp_TransactionDetails.created_at >= '$fromdate' and  temp_TransactionDetails.created_at <= '$todate'";
            else
               $condition = "temp_TransactionDetails.created_at >= '$fromdate'";       
        $data['transactions'] = $this->db->query("SELECT temp_TransactionDetails.*,students.firstname,students.admission_no,students.lastname 
        from temp_TransactionDetails 
        join students on students.id = temp_TransactionDetails.student_id
        where $condition")->result();   
    }else{
        $data['transactions'] =array();
       }
    ...
    $this->load->view('layout/header', $data);
    $this->load->view('studentfee/onlinepaymentattempt', $data);
    $this->load->view('layout/footer', $data);
    }
//--------------------------endclass
}
