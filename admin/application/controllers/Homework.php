...
class Homework extends Admin_Controller {
    ...
    public function index() {
		...
		$data['departmentList']=$this->Department_model->get_all();
     ...
     public function create() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $msg_type = $this->input->post("unicode");
            if($msg_type){
                $msg_type = 1;
            }
            else{
            ...
            if (!empty($departmentlisting)) { 
                ...
                foreach ($departmentlisting as $staffid)
                {                   
                    $numbers[] = $this->classteacher_model->get_data_by_query("SELECT contact_no as num from staff where id = '".$staffid."' ");
                }
            ...
            if (!empty($numbers)) {
                ...
                foreach($numbers as $num){ 
                   ...
                   $other_number.=','.$num[0]['num'];
                   
                }
                
             }
            // if (!empty($departmentlisting)) { foreach ($departmentlisting as $userlisting_key => $userlisting_value) {
   //                      if($userlisting_value)
   //                          {
   //                           $arr[]=$userlisting_value;
   //                              $dep_list = implode(',',$arr); 
   //                          }
...
//            $user_array = array();
//                $userlisting = $this->student_model->searchByClassSection($class_id, $section);
...
//              'subject_id' => form_error('subject_id'),
            ...
            $class_id=$this->input->post("class_id");
            $message=$this->input->post("description");
            $msg_id  =  $this->db->get_where('homework',array('id'=>$id))->row();
            $data1 = array(
                            'id' => $msg_id->msg_id,
                            'is_class' => 1,
                            'title' => 'Homework',
    ...
    $result = $this->homework_model->get_students_all();
    $data["result"] = $result;    
    print_r($result);
