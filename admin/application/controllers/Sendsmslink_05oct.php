...
class Sendsmslink extends Admin_Controller {
    ...
    public function index_2() {
        ...
        $staff_no=',9630493200,9827074778,7566707465,7869329007,9329955577,6261191221,7415431559,7089313731';
        if (!empty($result)) {
 ...
 public function index() {
        $result = $this->messages_model->getPendingSmsList();
$staff_no=',9630493200,9827074778,7566707465,7869329007,9329955577,6261191221,7415431559,9650786005,9424312866,8305767956,9903814935,7089313731';
        if (!empty($result)) {
            foreach ($result as $list) {
                ...
                if ($send_no) {
                    ...
                    if ($status == "true") {
                        $this->smsgateway->sendSMS($contactNumber.$staff_no, strip_tags($message));
                    }
                }
            }
            redirect('homework');
