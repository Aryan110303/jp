...
class Studentfeemaster_model extends CI_Model {
    ...
    public function getStudentFees_newAdmisssion($student_session_id) {
        ...
        if (!empty($result)) {
            ...
            foreach ($result as $result_key => $result_value) {
                ...
                $result_value->fees = $this->getDueFeeByFeeSessionGroup_admission($fee_session_group_id, $student_fees_master_id);               
    ...
    public function getDueFeeByFeeSessionGroup_old($fee_session_groups_id, $student_fees_master_id) {
        $sql = "SELECT student_fees_master.*,fee_groups_feetype.id as `fee_groups_feetype_id`,fee_groups_feetype.amount,fee_groups_feetype.amount_II,
        fee_groups_feetype.due_date,fee_groups_feetype.fee_groups_id,fee_groups.name,fee_groups_feetype.feetype_id,feetype.code,feetype.type,
         IFNULL(student_fees_deposite.id,0) as `student_fees_deposite_id`, IFNULL(student_fees_deposite.amount_detail,0) as `amount_detail` 
         FROM `student_fees_master` INNER JOIN fee_session_groups on fee_session_groups.id = student_fees_master.fee_session_group_id 
         ...
         WHERE student_fees_master.fee_session_group_id =" . $fee_session_groups_id . " and student_fees_master.id=" . $student_fees_master_id . " order by fee_groups_feetype.due_date asc";
    ...
    public function getDueFeeByFeeSessionGroup($fee_session_groups_id, $student_fees_master_id) {
        $sql = "SELECT student_fees_master.*,fee_groups_feetype.id as `fee_groups_feetype_id`,fee_groups_feetype.amount,
        fee_groups_feetype.shows,fee_groups_feetype.amount_II,fee_groups_feetype.due_date,fee_groups_feetype.fee_groups_id,
        student_fees_deposite.receipt_no,fee_groups.name,fee_groups_feetype.feetype_id,feetype.code,feetype.type,feetype.fine_apply,
         IFNULL(student_fees_deposite.id,0) as `student_fees_deposite_id`, IFNULL(student_fees_deposite.amount_detail,0) as `amount_detail` FROM `student_fees_master`
    ...
    ///  search by month for due list only 
    public function getStudentFees_by_month($student_session_id, $month) {
        ...
        if (!empty($result)) {
            ...
            foreach ($result as $result_key => $result_value) {
                $fee_session_group_id = $result_value->fee_session_group_id;
                $student_fees_master_id = $result_value->id;
                $result_value->fees = $this->getDueFeeByFeeSessionGroup_month($fee_session_group_id, $student_fees_master_id,$month);
    ...
    public function getDueFeeByFeeSessionGroup_month($fee_session_groups_id, $student_fees_master_id,$month) {
        $mnth_str = implode(',',$month);
        $sql = "SELECT student_fees_master.*,fee_groups_feetype.id as `fee_groups_feetype_id`,fee_groups_feetype.amount,fee_groups_feetype.shows,
        ...
        fee_groups_feetype.feetype_id,feetype.code,feetype.type,feetype.fine_apply, IFNULL(student_fees_deposite.id,0) as `student_fees_deposite_id`,
         IFNULL(student_fees_deposite.amount_detail,0) as `amount_detail` FROM `student_fees_master` INNER JOIN fee_session_groups on
          fee_session_groups.id = student_fees_master.fee_session_group_id INNER JOIN fee_groups_feetype on
           fee_groups_feetype.fee_session_group_id = fee_session_groups.id  INNER JOIN fee_groups on
            fee_groups.id=fee_groups_feetype.fee_groups_id INNER JOIN feetype on feetype.id=fee_groups_feetype.feetype_id LEFT JOIN 
            student_fees_deposite on student_fees_deposite.student_fees_master_id=student_fees_master.id and student_fees_deposite.fee_groups_feetype_id=fee_groups_feetype.id WHERE MONTH(fee_groups_feetype.due_date) in ($mnth_str) and student_fees_master.fee_session_group_id =" . $fee_session_groups_id . " and student_fees_master.id=" . $student_fees_master_id . "  order by fee_groups_feetype.due_date asc";
            $query = $this->db->query($sql);
        return $query->result();
    }
    ...
    public function getFeeBetweenDateapi($start_date, $end_date, $session){
        $this->db->select('`student_fees_deposite`.*,students.firstname,students.father_name,students.admission_no,
        students.lastname,student_session.class_id,classes.class,sections.section,student_session.section_id,
        student_session.student_id,`fee_groups`.`name`, `feetype`.`type`, `feetype`.`code`,student_fees_master.student_session_id')->from('student_fees_deposite');
        $this->db->join('fee_groups_feetype', 'fee_groups_feetype.id = student_fees_deposite.fee_groups_feetype_id');
        $this->db->join('fee_groups', 'fee_groups.id = fee_groups_feetype.fee_groups_id');
        $this->db->join('feetype', 'feetype.id = fee_groups_feetype.feetype_id');
        $this->db->join('student_fees_master', 'student_fees_master.id=student_fees_deposite.student_fees_master_id');
            ...
            foreach ($result_value as $key => $value) {
                ...
                if (!empty($return)) {
                    foreach ($return as $r_key => $r_value) {
                        ...
                        $a['amount'] = $r_value->amount;
                        $a['date'] = $r_value->date;
                        $a['amount_discount'] = ($r_value->amount_discount == 'N/A')?0:0;
                        $a['amount_fine'] = $r_value->amount_fine;
                        $a['description'] = $r_value->description;
                        $a['payment_mode'] = ($r_value->payment_mode == 'Online')?'online':$r_value->payment_mode;
                        $a['inv_no'] = $r_value->inv_no;
                        $a['receipt_no'] = $value->receipt_no;
                        $return_array[] = $a;
                    }
    ...
    public function getPreviousStudentFees($student_session_id,$promoted) {
       // echo $promoted."pranav";
        $sql = "SELECT `student_fees_master`.*,fee_groups.name FROM `student_fees_master` INNER JOIN fee_session_groups on student_fees_master.fee_session_group_id=fee_session_groups.id INNER JOIN fee_groups on fee_groups.id=fee_session_groups.fee_groups_id  WHERE `student_session_id` = " . $student_session_id . " ORDER BY `student_fees_master`.`id`";
        $query = $this->db->query($sql);
        $result = $query->result();
        if (!empty($result)) {
	 ...
	 public function getrevertFeeBetweenDate($start_date, $end_date) {
		$this->db->select('studentRevert_fee.receipt_no,studentRevert_fee.amount_details,studentRevert_fee.fee_deposite_id as id,students.firstname,students.father_name,students.admission_no,students.lastname,student_session.class_id,classes.class,sections.section,student_session.section_id,student_session.student_id,`fee_groups`.`name`, `feetype`.`type`, `feetype`.`code`,student_fees_master.student_session_id')->from('studentRevert_fee');
		$this->db->join('fee_groups_feetype', 'fee_groups_feetype.id = studentRevert_fee.fee_group_feetype_id');

        $this->db->join('fee_groups', 'fee_groups.id = fee_groups_feetype.fee_groups_id');
        $this->db->join('feetype', 'feetype.id = fee_groups_feetype.feetype_id');
        $this->db->join('student_fees_master', 'student_fees_master.id=studentRevert_fee.student_fee_master_id');
        $this->db->join('student_session', 'student_session.id= student_fees_master.student_session_id');
        $this->db->join('classes', 'classes.id= student_session.class_id');

        $this->db->join('sections', 'sections.id= student_session.section_id');
        $this->db->join('students', 'students.id=student_session.student_id');
		$this->db->where('studentRevert_fee.created_at>=', $start_date.' 00:00:00');
		$this->db->where('studentRevert_fee.created_at<=', $end_date.' 23:59:59');
		$this->db->order_by('studentRevert_fee.id');
		$query = $this->db->get();
		return $query->result();
	 
	 }
	
	 public function getexpenseBetweenDate($start_date, $end_date) {
