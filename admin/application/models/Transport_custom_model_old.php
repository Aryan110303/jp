...
//        $this->db->where('fee_session_group_id', $data['fee_session_group_id']);
    ...
    public function getFeeByInvoice1($invoice_id, $sub_invoice_id) {
        $this->db->select('`student_transportfees_deposite`.*,students.firstname,students.lastname,student_session.class_id,classes.class,sections.section,student_session.section_id,student_session.student_id,`fee_groups`.`name`, `transport_feetype`.`type`, `transport_feetype`.`code`,student_transportfees_master.student_session_id')->from('student_transportfees_deposite');
