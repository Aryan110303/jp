...
class Teachersubject_model extends CI_Model {
    ...
    public function getDetailbyClsandSection($class_id, $section_id, $exam_id) {
        $query = $this->db->query("SELECT teacher_subjects.*,exam_schedules.date_of_exam,exam_schedules.start_to,exam_schedules.end_from,exam_schedules.room_no,exam_schedules.full_marks,exam_schedules.passing_marks,subjects.name,
            ...
            ON teacher_subjects.class_section_id = class_sections.id WHERE teacher_subjects.session_id = " .$this->current_session. " and  class_sections.class_id =" . $this->db->escape($class_id) . " and class_sections.section_id=" . $this->db->escape($section_id));
 //echo $this->db->last_query(); die;
    ...
    public function getAllSection($class_id)
    {
        $this->db->select('*');
    ...
    public function getTeacherSubjectId($teacher_id,$subject_id)
    {
        $this->db->select('id');
        $this->db->from('teacher_subjects');
    ...
    public function getClassSection($classSecId,$subject_id,$teacher_id)
    {
        $this->db->select('id');
        $this->db->from('teacher_subjects');
        $this->db->where('class_section_id',$classSecId);
        $this->db->where('teacher_id',$teacher_id);
        $this->db->where('subject_id',$subject_id);
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
