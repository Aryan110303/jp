...
class User_model extends CI_Model {
 ...
 /*    public function checkLogin($data) {
    ...
    public function checkLogin($data) {
        $this->db->select('users.id, users.username, users.password,users.role,users.is_active');
        $this->db->from('users');
        $this->db->join('students', 'students.id = users.user_id');
        $this->db->like('students.mobileno', $data['username'] );
        $this->db->where('students.password', ($data['password']));
        $this->db->limit(1);
        $query = $this->db->get();
