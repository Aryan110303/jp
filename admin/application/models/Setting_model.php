...
class Setting_model extends CI_Model {
    ...
    public function get($id = null) {
        $this->db->select('sch_settings.id,sch_settings.lang_id,sch_settings.class_teacher,sch_settings.is_rtl,sch_settings.cron_secret_key, sch_settings.timezone,
          ...
          sch_settings.address,sch_settings.dise_code,sch_settings.date_format,sch_settings.currency,sch_settings.currency_symbol,sch_settings.start_month,sch_settings.session_id,sch_settings.fee_due_days,sch_settings.fine_amount,sch_settings.image,sch_settings.theme,sessions.session'
        );
    ...
    {
      $this->db->select('room_settings.roompath');
      $this->db->from('room_settings');
      $this->db->where('room_settings.is_active',1);
      $query = $this->db->get();
      return $query->row()->roompath;
    }

    public function getroomsecretkey( )
    {
        $this->db->select('room_settings.secretkey');
        $this->db->from('room_settings');
        $this->db->where('room_settings.is_active',1);
        $query = $this->db->get();
        return $query->row()->secretkey;
    }
