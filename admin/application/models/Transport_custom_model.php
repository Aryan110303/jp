  ...
  public function __construct() {
    ...
    public function remove1($id, $sub_invoice) {
        ...
        if ($q->num_rows() > 0) {
            ...
            if($a[$sub_invoice]){
                $s_id=$a[$sub_invoice]['transportfee_id'];
...
//        $this->db->where('fee_session_group_id', $data['fee_session_group_id']);
    ...
    public function getFeeByInvoice1($invoice_id, $sub_invoice_id) {
        ...
        $this->db->join('trans_fee_groups_feetype', 'trans_fee_groups_feetype.id = student_transportfees_deposite.fee_groups_feetype_id','left');
        $this->db->join('transport_route', 'transport_route.id = trans_fee_groups_feetype.fee_groups_id','left');
        $this->db->join('transport_feetype', 'transport_feetype.id = trans_fee_groups_feetype.feetype_id','left');
	...
	public function getFeeByInvoice2($invoice_id, $sub_invoice_id) {
        $this->db->select('`student_transportfees_deposite`.*,staff.name,staff.surname,student_session.class_id,student_session.section_id,student_session.student_id,`transport_feetype`.`type`, `transport_feetype`.`code`,student_transportfees_master.student_session_id')->from('student_transportfees_deposite');
        $this->db->join('trans_fee_groups_feetype', 'trans_fee_groups_feetype.id = student_transportfees_deposite.fee_groups_feetype_id','left');
        $this->db->join('transport_route', 'transport_route.id = trans_fee_groups_feetype.fee_groups_id','left');
        $this->db->join('transport_feetype', 'transport_feetype.id = trans_fee_groups_feetype.feetype_id','left');
        $this->db->join('student_transportfees_master', 'student_transportfees_master.id=student_transportfees_deposite.student_fees_master_id');
        $this->db->join('student_session', 'student_session.id= student_transportfees_master.student_session_id');
        $this->db->join('staff', 'staff.id=student_session.student_id');
        $this->db->where('student_transportfees_deposite.id', $invoice_id);
        $q = $this->db->get();
...
WHERE student_transportfees_master.id=".$student_fees_master_id;
     ...
     public function getDueFeeByFeeSessionGroupFeetype_staff($student_fees_master_id) {

//        $sql = "SELECT student_transportfees_master.*,students.firstname,students.lastname,student_session.class_id,classes.class,sections.section,students.guardian_name,students.father_name,student_session.section_id,student_session.student_id,trans_fee_groups_feetype.amount,trans_fee_groups_feetype.due_date,trans_fee_groups_feetype.fee_groups_id,transport_route.route_title,trans_fee_groups_feetype.feetype_id,transport_feetype.code,transport_feetype.type,
//          IFNULL(student_transportfees_deposite.id,0) as `student_fees_deposite_id`, IFNULL(student_transportfees_deposite.amount_detail,0) as `amount_detail` FROM `student_transportfees_master` INNER JOIN trans_fee_session_groups on trans_fee_session_groups.id = student_transportfees_master.fee_session_group_id INNER JOIN trans_fee_groups_feetype on  trans_fee_groups_feetype.fee_session_group_id = trans_fee_session_groups.id  INNER JOIN 
//          transport_route on transport_route.id=trans_fee_groups_feetype.fee_groups_id INNER JOIN transport_feetype on transport_feetype.id=trans_fee_groups_feetype.feetype_id LEFT JOIN student_transportfees_deposite on student_transportfees_deposite.student_fees_master_id=student_transportfees_master.id and student_transportfees_deposite.fee_groups_feetype_id=trans_fee_groups_feetype.id INNER JOIN student_session on student_session.id= student_transportfees_master.student_session_id INNER JOIN classes on classes.id= student_session.class_id INNER JOIN sections on sections.id= student_session.section_id INNER JOIN students on students.id=student_session.student_id  WHERE student_transportfees_master.fee_session_group_id =" . $fee_session_groups_id . " and student_transportfees_master.id=" . $student_fees_master_id . " and trans_fee_groups_feetype.id= " . $fee_groups_feetype_id;
//        
         $sql = "SELECT student_transportfees_master.*,IFNULL(student_transportfees_deposite.id,0) as `student_fees_deposite_id`, IFNULL(student_transportfees_deposite.amount_detail,0) as `amount_detail`,student_session.class_id, student_session.section_id,student_session.student_id,staff.name,staff.surname,staff.father_name,transport_route.route_title
...
INNER JOIN staff on staff.id=student_session.student_id 
...
WHERE student_transportfees_master.id=" . $student_fees_master_id;
    ...
    public function studenttransaction($student_fees_master_id) {
        $this->db->select('students.firstname,students.lastname,students.father_name,students.admission_no,students.roll_no,student_transportfees_master.student_session_id,student_transportfees_master.fees_amount,student_transportfees_deposite.*')->from('students');
        $this->db->join('student_session', 'students.id = student_session.student_id');
        $this->db->join('student_transportfees_master', 'student_session.id=student_transportfees_master.student_session_id');
        $this->db->join('student_transportfees_deposite', 'student_transportfees_master.id= student_transportfees_deposite.student_fees_master_id','left');
        $this->db->where('student_transportfees_master.id', $student_fees_master_id);
        $row= $this->db->get()->row();
        if(!empty($row)){
           $result= $this->student_transportfee_details($row->student_session_id);
           $total_amount=0;
            foreach ($result as $value) {
                
               $total_amount+= $value->fees_amount;
    ...
    public function student_transportfee_details($student_session_id) {
        $this->db->select('*');
        $this->db->from('student_transportfees_master');
        $this->db->where('student_session_id',$student_session_id);
        return $this->db->get()->result();
    }
    public function student_transportfee_deposite_details($student_fees_master_id) {
        $this->db->select('*');
        $this->db->from('student_transportfees_deposite');
     ...
     public function searchByroute1($route_id = null) {
        ...
        $this->db->select('student_session.id as student_session_id,staff.*')->from('staff');
        $this->db->join('student_session', 'student_session.student_id = staff.id');
        ...
        $this->db->where('student_session.user_type', 2);
    ...
    // public function student_fees_details($date = null) {
    //     $this->db->select('students.firstname,students.lastname,students.father_name,students.admission_no,students.roll_no,student_transport_fees.*,student_session.*,classes.class,sections.section,vehicles.vehicle_no')->from('student_transport_fees');
    ...
    //     $this->db->join('classes', 'classes.id=student_session.class_id');
    //     $this->db->join('sections', 'sections.id=student_session.section_id');
    //     $this->db->join('vehicles', 'vehicles.id=student_session.vehicle_id');
    //     if ($date != null) {
    //         $this->db->where('student_transport_fees.date', $date);
    //     }
    //     return $this->db->get()->result();
    ...
    // }
          ...
          public function student_fees_details($date = null) {
        $this->db->select('students.firstname,students.lastname,students.father_name,students.admission_no,students.roll_no,student_transport_fees.*,student_transport_fees.created_at as trans_date,student_session.*,classes.class,sections.section,vehicles.vehicle_no');
        $this->db->from('student_session');
        $this->db->join('student_transport_fees', 'student_transport_fees.student_session_id=student_session.id', 'left');
        ...
        $this->db->join('vehicles', 'vehicles.id=student_session.vehicle_id', 'left');
        if ($date != null) {
           $this->db->where('student_transport_fees.date',$date);
        }
        return   $this->db->get()->result();
