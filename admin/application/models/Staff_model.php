...
class Staff_model extends CI_Model {
    ...
    public function get_1($id = null) {

        $this->db->select('staff.*,roles.name as user_type,roles.id as role_id,student_session.vehicle_id,student_session.route_id,student_session.transport_fees,student_session.id as student_session_id')->from('staff')->join("staff_roles", "staff_roles.staff_id = staff.id", "left")->join("roles", "staff_roles.role_id = roles.id", "left");
		$this->db->join('student_session', 'student_session.student_id = staff.id','left');
    ...
    public function checkteacherLogin($data) {

        $record = $this->getBynumber($data['number'],$data['password']);
...
//        print_r($record);
    ...
    public function getBynumber($number,$password) {
        $this->db->select('*');
        $this->db->from('staff');
        $this->db->where('contact_no', $number);
...
//            die;
	...
	public function getBYdeparment($dep=''){
 ...
 public function getdetails_by_teacherID($value)
    {
            $qry = "SELECT teacher_timetable.*, staff.name as tname, staff.surname, subjects.name as subname, sections.section, classes.class  from teacher_timetable left join staff on teacher_timetable.teacher_id = staff.id left join sections on sections.id = teacher_timetable.section_id left join classes on teacher_timetable.class_id = classes.id  left join subjects on teacher_timetable.subject_id = subjects.id where staff.id = $value order by teacher_timetable.days, teacher_timetable.period_id asc  " ;
              $result = $this->db->query($qry)->result();
            return $result ;  
    } 
    
 public function getdetails_by_classID11($class,$section,$days_key,$period_value)
    {
            $qry = "SELECT teacher_timetable.*, staff.name as tname, staff.surname, subjects.name as subname, sections.section, classes.class  from teacher_timetable  join staff on teacher_timetable.teacher_id = staff.id  join sections on sections.id = teacher_timetable.section_id  join classes on teacher_timetable.class_id = classes.id   join subjects on teacher_timetable.subject_id = subjects.id where teacher_timetable.section_id = $section  and teacher_timetable.class_id = $class and period_id= $period_value and days=$days_key"  ;
                $result = $this->db->query($qry)->result();
                return $result ;  
    }  
...
public function getdetails_by_classID12($class,$section,$days_key,$period_value)
    {
            $qry = "SELECT teacher_timetable.*, staff.name as tname, staff.surname, subjects.name as subname, sections.section, classes.class  from teacher_timetable  join staff on teacher_timetable.teacher_id = staff.id  join sections on sections.id = teacher_timetable.section_id  join classes on teacher_timetable.class_id = classes.id   join subjects on teacher_timetable.subject_id = subjects.id where teacher_timetable.section_id = $section  and teacher_timetable.class_id = $class and period_id= $period_value and days=$days_key"  ;
    ...
    public function getdetails_by_classID12mob($class,$section,$days_key,$period_value)
    {
            $qry = "SELECT teacher_timetable.*, staff.name as tname, staff.surname, subjects.name as subname, sections.section, classes.class 
            from teacher_timetable  join staff on teacher_timetable.teacher_id = staff.id  join sections on sections.id = teacher_timetable.section_id 
            join classes on teacher_timetable.class_id = classes.id   join subjects on teacher_timetable.subject_id = subjects.id 
            where teacher_timetable.class_id = $class and period_id= $period_value and days=$days_key"  ;
                $result = $this->db->query($qry)->row();
                return $result ;  
    }  
public function getdetails_by_teacherID1($pid)
     {
             $this->db->where('teacher_id', $pid);
             $this->db->order_by('id');
              $result = $this->db->get('teacher_timetable')->result_array();
             return $result ;  
     }
...
{
    $qry = "SELECT tt.* ,st.name as tname,st.surname, sub.name as subname , c.class, sec.section from teacher_timetable tt , staff st, subjects sub, classes c, sections sec where  tt.teacher_id = st.id and tt.class_id = c.id and tt.section_id = sec.id and tt.subject_id = sub.id ";
